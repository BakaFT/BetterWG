...
typedef int (*TCefBrowserHostCreateBrowser)(
    ...
    client->get_request_handler = [](cef_client_t* self)-> cef_request_handler_t*
        {
            ...
            (
                ...
                {                    
                    auto resourceRequestHandler = GetResourceRequestHandler(self, browser, frame, request, is_navigation, is_download, request_initiator, disable_default_handling);
                    ...
                    resourceRequestHandler->get_resource_response_filter = [](
                        ...
                        -> cef_response_filter_t* 
                        {
                            ...
                            // https://wegame.gtimg.com/g.26-r.c2d3c/helper/lol/v2/assets/battle-detail-base.4c9a3294.js
                            // length = 89
                            std::wstring suffix = L"4c9a3294.js";                         

                            // If length of url is 89 and ends with suffix, then that's the file we want to intercept
                            if (urlWstr.length() == 89 &&
                                urlWstr.compare(urlWstr.length() - suffix.length(), suffix.length(), suffix) == 0) 
                            {                                                                                              
...
static int Hooked_CefBrowserHost_CreateBrowser(const cef_window_info_t* windowInfo,
    struct _cef_client_t* client,
    const cef_string_t* url,
    const struct _cef_browser_settings_t* settings,
    struct _cef_dictionary_value_t* extra_info,
...
void HookCefBroswserHostCreate() {
    fpOriginal_filter = reinterpret_cast<TCefBrowserHostCreateBrowser>(helper::get_module_export("libcef.dll", "cef_browser_host_create_browser"));
