...
private:
    ...
    static cef_response_filter_status_t CEF_CALLBACK _filter(
        ...
        size_t* data_out_written)
    {
        ...
        const std::string insert_str = "Y(P(et),{text:'TeamELO'},{default:Q(()=>[h('div',Bn,['TeamELO:',h('span',null,w(o.battleDetailTeamData.teamElo),1)])]),_:1}),";
        ...
        if (pos != std::string::npos) {
            ...
            memcpy(static_cast<char*>(data_out) + pos + fragment.length(), static_cast<char*>(data_in) + pos + target_str.length(), data_in_size - pos - target_str.length());
